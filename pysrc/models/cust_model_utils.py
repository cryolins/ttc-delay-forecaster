...
NUM_FEATURES = ["min_gap", "route_stops", "route_length", "dt_dist_mean", "dt_dist_std", 
...
def split_data(df: pd.DataFrame, incl_val=True):
    if not incl_val:
...
def save_metrics(metrics: dict, name):
    with open(DATA_DIR_PATH / "metrics" / f"{name}.json", "w") as f:
...
def visualize_preds(test: pd.DataFrame, y_test, y_pred, img_name = None):
    ...
    axes[1].set_title("Baseline: Residuals Over Time")

    plt.tight_layout()
    if img_name:
        plt.savefig(DATA_DIR_PATH / "metrics" / f"{img_name}.png", dpi=150)
    else:
        plt.show()
